  ...
  for (i = 0; i < semesters.length; i++) {
    if ($(semesters[i]).children().length < 5) {
      ...
      if (typeof name === "string") {
        ...
        $(newClass).html(name[0]);
        newClass.classList.add('hvr-curl-top-right');
        $(newClass).click(function () {
          ...
          for (j = 1; j < name.length; j++) {
            ...
            var opClass = document.createElement('DIV');
            var xBut = document.createElement('I');
            xBut.classList.add('js-remove');
            xBut.innerHTML = 'X';
            opClass.classList.add('op_box');
            opClass.appendChild(xButton);
            console.log("ended for");
          }}else{alert("over 10 choices look at academic worksheet");}
        });
      }
...
function selectMajor(major) {
  loadJSON(function (response) {
    ...
    if (major == "POLI") { loadClasses2(majorJSON.POLI); }
...
function loadClasses2(major) {//Checks all the JSON parts and fills them if the category exists
  var color = chooseColor(major);
  if (major.absolute_courses != null) {
    for (j = 0; j < major.absolute_courses.length; j++) {
      addClass(major.absolute_courses[j], color);
  ...
  if (major.choice_courses != null) {
    var k = 0;
    for (j = 0; j < major.choice_courses.choices.length; j++) {
      var courseToAdd = major.choice_courses.choices[j];
      addClass(courseToAdd, color);
    }
  }
  if (major.range_courses != null) {
